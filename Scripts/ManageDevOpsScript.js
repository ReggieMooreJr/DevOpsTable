$(document).ready(function () {
    ...
    $('td[class^="type-"]').on('click', function (e) {
        e.preventDefault();
        //var id = $(this).data("pos");
        var title = $(this).find('span').html();
        //$("#description .modal-title").html(title);
   ...
   function mouseEventTable () {
          $('td[class^="type-"]').mouseover(function () {
              var currentClass = $(this).attr('class').split(' ')[0];
              if (currentClass != 'empty') {
                  $('.main > td').addClass('deactivate');
                  $('.' + currentClass).removeClass('deactivate');
              }
          });

          $('td[class^="cat-"]').mouseover(function () {
              var currentClass = $(this).attr('class').split(' ')[0];
              $('.main > td').addClass('deactivate');
              $('.' + currentClass).removeClass('deactivate');
          });

          $('.main > td').mouseout(function () {
              var currentClass = $(this).attr('class').split(' ')[0];
              $('.main > td').removeClass('deactivate');
          });
      }